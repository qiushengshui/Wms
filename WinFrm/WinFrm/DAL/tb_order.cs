...
namespace DAL
{
    ...
    public partial class tb_order
    {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select o_id,o_no,o_time,tp.p_no,tp.p_name,o_num,o_sum,h_name,h_tel,h_address,h_zipcode,o_user,o_desc,o_busy,o_type ");
            strSql.Append(" FROM tb_order ta inner join tb_kehu tb on ta.o_busy=tb.h_id inner join tb_proc tp on ta.o_pid=tp.p_id ");
            if (strWhere.Trim() != "")
            {
