        ...
        public List<Model.tb_order> DataTableToList(DataTable dt)
        {
            ...
            if (rowsCount > 0)
            {
                ...
                for (int n = 0; n < rowsCount; n++)
                {
                    ...
                    if (dt.Rows[n]["o_num"] != null && dt.Rows[n]["o_num"].ToString() != "")
                    {
                        model.o_num = int.Parse(dt.Rows[n]["o_num"].ToString());
                    }
                    if (dt.Rows[n]["o_pid"] != null && dt.Rows[n]["o_pid"].ToString() != "")
                    {
                        model.o_pid = int.Parse(dt.Rows[n]["o_pid"].ToString());
                    }
                    modelList.Add(model);
                }
            }
